...
const Question1 = () => {
  ...
  return (
    ...
    <div className={`${stat=="wrong"?"show":"hide"}`}><iframe src="https://lottie.host/?file=95e08e8c-8c5c-4e1a-8e52-66b1f9bc5c51/k75DUPenEI.json"></iframe></div>
      <NavLink className="navlink" to="/Question2"><span className={reveal=='true'?'nextques':'hide'}>&#9992;</span></NavLink>
    ...
    <div className="question" >
     "SUBH KAAM SHURU KARNE SE PEHLE KUCH MITHA HO JAYE" DO YOU REMEMBER WHOSE BIRTHDAY WAS THIS ??
    </div>

    <div className='set1'>
    <div className={`ques ${reveal=='true'?'cor':''}`} onClick={()=>{changeclass("correct")}}>MANISH'S 2023</div>
    <div className={`ques ${reveal=='true'?'wor':''}`} onClick={()=>{changeclass("wrong")}}>MINE 2022</div>
    </div>
    <div className='set2'>
    <div className={`ques ${reveal=='true'?'wor':''}`} onClick={()=>{changeclass("wrong")}}>YOURS 2022</div>
    <div className={`ques ${reveal=='true'?'wor':''}`} onClick={()=>{changeclass("wrong")}}>LIZA 2022</div>
    </div>
