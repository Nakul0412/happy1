...
const Question6 = () => {
  ...
  return (
    ...
    <div className={`${stat=="wrong"?"show":"hide"}`}><iframe src="https://lottie.host/?file=95e08e8c-8c5c-4e1a-8e52-66b1f9bc5c51/k75DUPenEI.json"></iframe></div>
      <NavLink className="navlink" to="/Question7"><span className={reveal=='true'?'nextques':'hide'}>&#9992;</span></NavLink>
    ...
    <div className="question" >
     ARE YOU ENJOYING THIS? HOPEFULLY YOU REMEBER THIS   WHAT WAS THE FIRST THING I TAUGHT YOU ? 
    </div>

    <div className='set1'>
    <div className={`ques ${reveal=='true'?'cor':''}`} onClick={()=>{changeclass("correct")}}>AVL TREES</div>
    <div className={`ques ${reveal=='true'?'wor':''}`} onClick={()=>{changeclass("wrong")}}>SQL</div>
    </div>
    <div className='set2'>
    <div className={`ques ${reveal=='true'?'wor':''}`} onClick={()=>{changeclass("wrong")}}>REACT</div>
    <div className={`ques ${reveal=='true'?'wor':''}`} onClick={()=>{changeclass("wrong")}}>BINARY SEARCH TREES</div>
    </div>
