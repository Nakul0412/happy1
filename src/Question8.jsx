...
const Question8 = () => {
  ...
  return (
    ...
    <div className={`${stat=="wrong"?"show":"hide"}`}><iframe src="https://lottie.host/?file=95e08e8c-8c5c-4e1a-8e52-66b1f9bc5c51/k75DUPenEI.json"></iframe></div>
      <NavLink className="navlink" to="/Question9"><span className={reveal=='true'?'nextques':'hide'}>&#9992;</span></NavLink>
    ...
    ENOUGH OF ME NOW LETS SEE WHETHER YOU REMEMBER ONLY YOUR BIRTHDAY OR YOU REMEMBER EVERONE'S BIRTHDAY IDENTIFY THIS PICTURE
    </div>

    <div className='set1'>
    <div className={`ques ${reveal=='true'?'wor':''}`} onClick={()=>{changeclass("wrong")}}>NIKET'S 2022</div>
    <div className={`ques ${reveal=='true'?'cor':''}`} onClick={()=>{changeclass("correct")}}>LUCKSHAY"S 2022</div>
    </div>
    <div className='set2'>
    <div className={`ques ${reveal=='true'?'wor':''}`} onClick={()=>{changeclass("wrong")}}>MAITRI'S 2021</div>
    <div className={`ques ${reveal=='true'?'wor':''}`} onClick={()=>{changeclass("wrong")}}>NOT A BIRTHDAY PICTURE</div>
    </div>
