...
const Question7 = () => {
  ...
  return (
    ...
    <div className={`${stat=="wrong"?"show":"hide"}`}><iframe src="https://lottie.host/?file=95e08e8c-8c5c-4e1a-8e52-66b1f9bc5c51/k75DUPenEI.json"></iframe></div>
      <NavLink className="navlink" to="/Question8"><span className={reveal=='true'?'nextques':'hide'}>&#9992;</span></NavLink>
    ...
    BETTER GET THIS RIGHT OTHERWISE THIS THING WILL STOP ONCE AND FOR ALL SO WHICH SONG ACTUALLY REMINDS YOU OF ME?
    </div>

    <div className='set1'>
    <div className={`ques ${reveal=='true'?'wor':''}`} onClick={()=>{changeclass("wrong")}}>DANDELIONS</div>
    <div className={`ques ${reveal=='true'?'wor':''}`} onClick={()=>{changeclass("wrong")}}>MILON CHALE</div>
    </div>
    <div className='set2'>
    <div className={`ques ${reveal=='true'?'cor':''}`} onClick={()=>{changeclass("correct")}}>I LIKE ME BETTER</div>
    <div className={`ques ${reveal=='true'?'wor':''}`} onClick={()=>{changeclass("wrong")}}>ALL SONGS</div>
    </div>
