...
const Question4 = () => {
  ...
  return (
    ...
    <div className={`${stat=="wrong"?"show":"hide"}`}><iframe src="https://lottie.host/?file=95e08e8c-8c5c-4e1a-8e52-66b1f9bc5c51/k75DUPenEI.json"></iframe></div>
      <NavLink className="navlink" to="/Question5"><span className={reveal=='true'?'nextques':'hide'}>&#9992;</span></NavLink>
    ...
    <div className="question" >
      THIS IS A TOUGH ONE  SO ANSWER CAREFULLY WHAT WAS THE FIRST TEXT I SENT ON WHATSAPP?
    </div>

    <div className='set1'>
    <div className={`ques ${reveal=='true'?'wor':''}`} onClick={()=>{changeclass("wrong")}} style={{fontSize:"40px"}}>Kal tere favourite jagaha par aa raha he singer 🥳🥳</div>
    <div className={`ques ${reveal=='true'?'cor':''}`} onClick={()=>{changeclass("correct")}}>UP?</div>
    </div>
    <div className='set2'>
    <div className={`ques ${reveal=='true'?'wor':''}`} onClick={()=>{changeclass("wrong")}}>Heyy Nakul this Side!</div>
    <div className={`ques ${reveal=='true'?'wor':''}`} onClick={()=>{changeclass("wrong")}}>HI! Meghna Its me</div>
    </div>
